...
1. You will get a pre-populated LearnR `.Rmd` document. We'll start by keeping the YAML frontmatter, the setup code chunk, and one of the regular LearnR code chunks (containing `exercise = TRUE`)---and now **delete** everything else. We can only create one LearnR window per LearnR `.Rmd`, so this is why we are only keeping a single exercise chunk in this document. 
...
1. Paste in the following script tags to the HTML file, underneath the previous code you added. This is the homemade JavaScript--that will let the iFrame resize *even when* the LearnR window produces content that spills outside of the window's bounds (e.g. when a Solutions tab is opened up). 
...
A couple things to point out about the code above:
  ...
  * The stuff following the "style" attribute is a bit of inline CSS here. This centers the iframe on the page, which I strongly recommend. Technically, it's better form to put all CSS in its own file, but I'm writing inline here for the sake of simplicity. 
